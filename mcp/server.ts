...
const TOOLS = [
  ...
  { name: "analyze_media", description: "Kick off perception analysis for an asset (async, non-blocking). Default kinds ['shots']. Poll get_analysis_status / read with get_shots. Desktop app only.", inputSchema: obj({ assetId: S.str, kinds: S.strArr }, ["assetId"]) },
  { name: "get_analysis_status", description: "Per-kind analysis status for an asset: ready | pending | none | unsupported.", inputSchema: obj({ assetId: S.str, kinds: S.strArr }, ["assetId"]) },
