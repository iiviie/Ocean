...
const TOOLS = [
  ...
  { name: "analyze_media", description: "Kick off perception analysis for an asset (async, non-blocking). Default kinds ['shots','silence']; also 'transcript','beats','waveform'. Poll get_analysis_status / read with get_shots/get_silence/get_transcript/get_beats/get_waveform. Desktop app only.", inputSchema: obj({ assetId: S.str, kinds: S.strArr }, ["assetId"]) },
  ...
  { name: "get_waveform", description: "Loudness/waveform envelope of an asset's audio as text: a coarse 0-100 curve plus loud spans, quiet spans, prominent hits, and peak/rms — reason about highs/lows and energy. Returns { status } if not ready (analyze_media kinds:['waveform']).", inputSchema: obj({ assetId: S.str, fromSec: S.num, toSec: S.num }, ["assetId"]) },
  { name: "get_beats", description: "Beat grid for an asset's audio: bpm, method ('aubio' real / 'energy' approximate), and beat timestamps in seconds — cut on the beat. Returns { status } if not ready (analyze_media kinds:['beats']).", inputSchema: obj({ assetId: S.str, fromSec: S.num, toSec: S.num }, ["assetId"]) },
  { name: "get_audio_summary", description: "One-shot audio perception: tempo (bpm + beats), loudness (peak/rms, loud & quiet spans, hits), silence, transcript availability. Start here for music/voice before pulling detail.", inputSchema: obj({ assetId: S.str }, ["assetId"]) },
  { name: "mark_beats", description: "Drop beat markers on the timeline from an asset's beat grid — fastest way to set up beat-synced cuts. clipId maps beats through that clip's placement/trim/speed; replace (default true) clears existing beat markers first. Returns { added }.", inputSchema: obj({ assetId: S.str, clipId: S.str, fromSec: S.num, toSec: S.num, replace: S.bool }, ["assetId"]) },
