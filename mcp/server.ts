...
const TOOLS = [
  ...
  { name: "set_text", description: "Edit a text clip: content/font/size/color/align/lineHeight + fontWeight (100-900), italic, letterSpacing (px), backgroundColor (plate), strokeColor, strokeWidth (px).", inputSchema: obj({ clipId: S.str, content: S.str, fontName: S.str, fontSize: S.num, color: S.str, align: { type: "string", enum: ["left", "center", "right"] }, lineHeight: S.num, fontWeight: S.num, italic: S.bool, letterSpacing: S.num, backgroundColor: S.str, strokeColor: S.str, strokeWidth: S.num }, ["clipId"]) },
  { name: "set_color", description: "Color-correct a video/image clip: brightness/contrast (-1..1, 0=none), saturation (0..2, 1=none), hue (deg), filter preset (none|grayscale|sepia|invert|vintage).", inputSchema: obj({ clipId: S.str, brightness: S.num, contrast: S.num, saturation: S.num, hue: S.num, filter: { type: "string", enum: ["none", "grayscale", "sepia", "invert", "vintage"] } }, ["clipId"]) },
  { name: "set_style", description: "Visual clip appearance/compositing: blendMode (normal|multiply|screen|overlay|darken|lighten|difference|add), cornerRadius (0..1), borderColor, borderWidth (px), shadow (bool).", inputSchema: obj({ clipId: S.str, blendMode: { type: "string", enum: ["normal", "multiply", "screen", "overlay", "darken", "lighten", "difference", "add"] }, cornerRadius: S.num, borderColor: S.str, borderWidth: S.num, shadow: S.bool }, ["clipId"]) },
  { name: "set_opacity", description: "Set clip opacity 0..1.", inputSchema: obj({ clipId: S.str, opacity: S.num }, ["clipId", "opacity"]) },
  { name: "set_fade", description: "Fade a clip in/out (seconds) — opacity for visual clips, volume for audio clips.", inputSchema: obj({ clipId: S.str, fadeInSec: S.num, fadeOutSec: S.num }, ["clipId"]) },
